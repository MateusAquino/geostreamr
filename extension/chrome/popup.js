...
async function loadSensitivePreference() {
  try {
    const stored = await chrome.storage.local.get(STORAGE_KEYS.hideMode);
    const value =
...
function handlePopupUnload() {
  ...
  if (state.connectedTabId) {
    chrome.tabs.sendMessage(state.connectedTabId, {
...
async function findGeoGuessrTab() {
  ...
  const candidates = await chrome.tabs.query({ url: GEO_TAB_URL_MATCH });
