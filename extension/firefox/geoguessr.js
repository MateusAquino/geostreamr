...
const BROADCAST_IDLE_TIMEOUT = 1000;
...
function scheduleBroadcast(forceImmediate = false) {
  ...
  const delay = forceImmediate
    ? 0
    : elapsed >= MIN_BROADCAST_INTERVAL
    ? 0
    : MIN_BROADCAST_INTERVAL - elapsed;
  ...
  const queueBroadcast = () => {
    ...
    const runBroadcast = () => {
      ...
      try {
        ...
        console.debug("GeoStreamr mirror broadcast scheduling failed:", error);
      }
    };
    ...
    const idleCallback =
      typeof window.requestIdleCallback === "function"
        ...
        : null;
    if (idleCallback) {
      idleCallback(runBroadcast, { timeout: BROADCAST_IDLE_TIMEOUT });
    } else {
      window.requestAnimationFrame(runBroadcast);
    }
  };
  ...
  if (delay === 0) {
    ...
    broadcastTimerId = setTimeout(queueBroadcast, delay);
  }
}
...
function sendMirrorUpdate() {
  try {
    ...
    if (connected) {
      ...
      const signature = `${payload.available}|${payload.hiddenInPage}|${payload.disabled}|${payload.html}`;
